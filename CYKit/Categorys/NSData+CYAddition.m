...
- (nullable NSData *)cy_aes256EncryptWithKey:(NSData *)key iv:(NSData *)iv
{
    ...
    if (status == kCCSuccess) {
        result = [[NSData alloc]initWithBytes:buffer length:encryptSize];
        free(buffer);
        return result;
    }
    else{
...
- (nullable NSData *)cy_aes256DecryptWithKey:(NSData *)key iv:(NSData *)iv
{
    if (key.length != 16 && key.length != 24 && key.length != 32) {
        return nil;
    }
    ...
    CCCryptorStatus status = CCCrypt(kCCDecrypt,
                                     ...
                                     &decryptSize);
    if (status == kCCSuccess) {
        result = [NSData dataWithBytes:buffer length:decryptSize];
        free(buffer);
        return result;
    }
    else{
...
static const short base64DecodeTable[256] = {
...
- (nullable NSString *)cy_base64String
{
    ...
    for (i = 0; i < length; i += 3) {
        ...
        output[index + 0] = base64EncodeTable[(value >> 18) & 0x3F];
        output[index + 1] = base64EncodeTable[(value >> 12) & 0x3F];
        output[index + 2] = ((i + 1) < length)
        ? base64EncodeTable[(value >> 6) & 0x3F]
        : '=';
        output[index + 3] = ((i + 2) < length)
        ? base64EncodeTable[(value >> 0) & 0x3F]
        : '=';
    }
...
+ (NSData *)cy_dataWithBase64EncodeedString:(NSString *)base64EncodedString
{
    ...
    while (inputPoint < length) {
        ...
        output[outputPoint++] = (base64DecodeTable[i0] << 2)
        | (base64DecodeTable[i1] >> 4);
        if (outputPoint < outputLength) {
            output[outputPoint++] = ((base64DecodeTable[i1] & 0xf) << 4)
            | (base64DecodeTable[i2] >> 2);
        }
        if (outputPoint < outputLength) {
            output[outputPoint++] = ((base64DecodeTable[i2] & 0x3) << 6)
            | base64DecodeTable[i3];
        }
    }
